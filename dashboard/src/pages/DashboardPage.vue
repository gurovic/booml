    ...
    <main class="dashboard__main">
      ...
      <template v-else>
        ...
        <section class="panel logs-panel">
          <div class="panel__header">
            <div>
              <h2 class="panel__title panel__title--compact">Журналы системы</h2>
              <p class="panel__subtitle panel__subtitle--compact">Последние записи из app.log и errors.csv</p>
            </div>
          </div>

          <div v-if="logRefreshError" class="dashboard__inline-alert dashboard__inline-alert--soft">
            {{ logRefreshError }}
          </div>
          ...
          <div class="logs-grid">
            <article class="logs-card">
              <div class="logs-card__head">
                <h3 class="logs-card__title">errors.csv</h3>
                <span class="logs-card__count">{{ formatCount(errorLogEntries.length) }}</span>
              </div>
              <div v-if="errorLogEntries.length === 0" class="logs-card__empty">Нет записей</div>
              <div v-else class="logs-table-wrap">
                <table class="logs-table">
                  ...
                  <tbody>
                    <tr v-for="(row, idx) in errorLogEntries" :key="`error-${idx}`">
                      <td class="logs-table__timestamp">{{ row.timestamp || '—' }}</td>
                      <td><span class="logs-level-badge" :class="`logs-level-badge--${levelTone(row.level)}`">{{ row.level || 'N/A' }}</span></td>
                      <td class="logs-table__source">
                        {{ row.module || row.logger || '—' }}
                        <span v-if="row.lineno">:{{ row.lineno }}</span>
                      </td>
                      <td class="logs-table__message">{{ row.message || '—' }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <article class="logs-card">
              <div class="logs-card__head">
                <h3 class="logs-card__title">app.log</h3>
                <span class="logs-card__count">{{ formatCount(appLogEntries.length) }}</span>
              </div>
              <div v-if="appLogEntries.length === 0" class="logs-card__empty">Нет записей</div>
              <div v-else class="logs-table-wrap">
                <table class="logs-table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    <tr v-for="(row, idx) in appLogEntries" :key="`app-${idx}`">
                      <td class="logs-table__timestamp">{{ row.timestamp || '—' }}</td>
                      <td><span class="logs-level-badge" :class="`logs-level-badge--${levelTone(row.level)}`">{{ row.level || 'N/A' }}</span></td>
                      <td class="logs-table__source">{{ row.logger || '—' }}</td>
                      <td class="logs-table__message">{{ row.message || '—' }}</td>
                    </tr>
                  </tbody>
                </table>
...
const appLogEntries = computed(() => (logsPayload.value?.app_log || []).slice(0, 100))
const errorLogEntries = computed(() => (logsPayload.value?.error_log || []).slice(0, 100))
...
async function refreshLogs({ initial = false } = {}) {
  try {
    ...
    logsPayload.value = await getLogs()
  } catch (err) {
    const message = normalizeLogError(err)
    if (initial) {
      logRefreshError.value = `Логи временно недоступны: ${message}`
    } else {
      logRefreshError.value = `Не удалось обновить логи: ${message}`
    }
  }
}
...
function normalizeLogError(err) {
  if (!err?.message) {
    return 'ошибка сети'
  }

  if (err.message === 'HTTP 403') {
    return 'доступ к логам есть только у администратора'
  }
...
.dashboard__inline-alert--soft {
  margin-top: 16px;
  border-color: rgba(120, 137, 186, 0.2);
  background: rgba(95, 108, 146, 0.08);
  color: #4f5c81;
}
...
.logs-card {
  border: 1px solid rgba(223, 230, 251, 0.9);
  border-radius: 20px;
  background: linear-gradient(180deg, rgba(246, 248, 255, 0.74) 0%, rgba(255, 255, 255, 0.9) 100%);
  overflow: hidden;
}
...
.logs-level-badge--danger {
  background: rgba(255, 91, 77, 0.14);
  color: #b4443b;
}

.logs-level-badge--warning {
  background: rgba(247, 129, 40, 0.16);
  color: #b06b2f;
}

.logs-level-badge--info {
  background: rgba(89, 182, 223, 0.18);
  color: #3d7795;
}

.logs-level-badge--neutral {
  background: rgba(95, 108, 146, 0.14);
  color: #566285;
}
